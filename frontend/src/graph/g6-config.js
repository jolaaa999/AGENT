 ...
 */
export function preprocessGraphData(nodes, edges, options = {}) {
...
function styleNode(node, dimmed, degree) {
...
function styleEdge(edge, dimmed) {
...
export function buildStyledGraph(graph, focus) {
    ...
    const nodes = graph.nodes.map((node) => styleNode(node, hasFocus ? !focus.nodeIds.has(node.id) : false, degrees.get(node.id) ?? 0));
...
export function buildFocusSet(paths) {
