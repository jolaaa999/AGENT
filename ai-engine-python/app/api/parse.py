...
from app.schemas.graph import ExplainRequest, ExplainResponse, ParseRequest, ParseResponse
from app.services.deepseek_service import DeepSeekParseError, explain_concept, parse_relations_with_retry
...
def explain(payload: ExplainRequest) -> ExplainResponse:
    try:
        explanation = explain_concept(concept=payload.concept.strip(), markdown=payload.markdown)
    except DeepSeekParseError as exc:
        raise HTTPException(status_code=502, detail=str(exc)) from exc
    except Exception as exc:  # noqa: BLE001
        logger.exception("unexpected explain error")
        raise HTTPException(status_code=500, detail="unexpected explain error") from exc

    return ExplainResponse(concept=payload.concept.strip(), explanation=explanation)
